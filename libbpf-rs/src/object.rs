...
impl MapBuilder {
    ...
    pub fn load(&mut self) -> Result<Map> {
        ...
        let mut fd = unsafe { libbpf_sys::bpf_create_map_xattr(&self.attrs) };
        if fd < 0 && (self.attrs.btf_key_type_id > 0 || self.attrs.btf_value_type_id > 0) {
            // Retry without BTF
            self.attrs.btf_fd = 0;
            self.attrs.btf_key_type_id = 0;
            self.attrs.btf_value_type_id = 0;
            fd = unsafe { libbpf_sys::bpf_create_map_xattr(&self.attrs) };
        }
...
impl ProgramBuilder {
    ...
    pub fn load(&mut self) -> Result<Program> {
        let ret = unsafe {
            libbpf_sys::bpf_program__load(
                self.ptr,
                self.license.as_mut_str().as_mut_ptr() as *mut c_char,
                0,
            )
        };
        if ret != 0 {
            return Err(Error::Internal(format!("libbpf_errno={}", ret)));
        }
